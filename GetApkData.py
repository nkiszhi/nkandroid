...
def parseargs():
    ...
    parser.add_argument("-f", "--destination", type=str, default="www/test")
...
def GetFromXML(ApkDirectoryPath, ApkFile, DstPath):
    '''
    ...
    try:
        ...
        f = open(DstPath + os.path.basename(ApkFile).split('.')[0] + ".xml", "w")
        f.write(lxml.etree.tostring( a.xml['AndroidManifest.xml'], pretty_print=True).decode() )
        f.close()
    except Exception as e:
        print(e)
        logger.error(e)
        logger.error(DstPath + ApkFile + " to get AndroidManifest.xml Failed.")
        return
    try:
        ...
        logger.error(e)
        logger.error("Cannot resolve "  + "'s AndroidManifest.xml File!");
...
def ProcessingDataForGetApkData(ApkDirectoryPath, DstPath, ApkFile, PMap=PScoutMapping.PScoutMapping):
    '''
    ...
    '''
    CWD = os.getcwd()
    os.chdir(os.path.join(CWD, "/home/www/drebin/drebin/Modules"))
    ...
    try:
        ...
        IntentFilterSet = GetFromXML(ApkDirectoryPath, ApkFile, DstPath)
        ...
        # Set S6, S5, S7, S8
        CM.ExportToJson(DstPath + os.path.basename(ApkFile).split('.')[0] + ".data", DataDictionary)
    except Exception as e:
        FinalTime = time.time()
        logger.error(e)
...
def GetApkData(ProcessNumber, DstPath, *ApkDirectoryPaths):
    '''
    ...
    for ApkFile in ApkFileList:
        if CM.FileExist(DstPath + os.path.basename(ApkFile).split('.')[0] + ".data"):
            pass
        else:
            ...
            ProcessingResults = pool.apply_async(ProcessingDataForGetApkData, args=(ApkDirectoryPath, DstPath, ApkFile, PMap),
...
if __name__ == '__main__':
    ...
    DstDirectoryPaths = Args.destination
    GetApkData(ProcessNumber, DstDirectoryPaths, ApkDirectoryPaths)
